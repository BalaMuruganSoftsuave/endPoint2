...
class CharterCubit extends Cubit<CharterState> {
  ...
  getCharterList(String char)async{
    emit(state.copyWith(charterReqState: state.charterReqState=CharterReqState.LOADING));
    ...
    emit(state.copyWith(charterReqState: state.charterReqState=CharterReqState.DONE));
  }
  addCharter(String name, String email, String address, String country,
    String website, String contactPerson, String phoneNo,String state1, String city){
    emit(state.copyWith(charterReqState: state.charterReqState=CharterReqState.LOADING));
    var res = APIHelper().addCharter(name, email, address, country, website, contactPerson, phoneNo,state1,city);
    emit(state.copyWith(charterReqState: state.charterReqState=CharterReqState.DONE));
    return res;
  }
...
class CharterState {
  ...
  CharterState({this.charterList, this.charterReqState = CharterReqState.NONE,});
  CharterState copyWith(
      {CharterList? charterList, CharterReqState? charterReqState,}) {
    return CharterState(
